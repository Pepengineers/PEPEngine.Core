	...
	namespace JobSystem
	{
		...
		private:
			static constexpr void check_args()
			{
				...
				argument_checker<0, typename func_traits::args, std::tuple<Args...>>::check();
			}
		...
		struct delegate_size_checker
		{
			static constexpr void check()
			{
				static_assert(Actual <= Maximum, __FUNCTION__
					": Delegate exceeds size limit <Actual Size, Maximum Size>");
			}
		};
	}
}
